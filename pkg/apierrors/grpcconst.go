...
const (
	...
	// OutOfRange The operation was attempted past the valid range.  E.g., seeking or reading past end-of-file.
	// Unlike `INVALID_ARGUMENT`, this error indicates a problem that may be fixed if the system state changes.
	// For example, a 32-bit file system will generate `INVALID_ARGUMENT` if asked to read at an offset
	// that is not in the range [0,2^32-1], but it will generate `OUT_OF_RANGE` if asked to read from an offset past the current file size.
	...
	// Internal errors. It means that some invariants expected by the underlying system have been broken.
	// This error code is reserved for serious errors.
	// HTTP Mapping: 500 Internal Server Error
	Internal

	// Unavailable The service is currently unavailable.  This is most likely a transient condition,
	// which can be corrected by retrying with a backoff. Note that it is not always safe to retry non-idempotent operations.
	// See the guidelines above for deciding between `FAILED_PRECONDITION`, `ABORTED`, and `UNAVAILABLE`.
